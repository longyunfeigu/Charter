/* Charter · 方案 D 混合终案 interactions(A 抽屉 + C 落点)。NOT product code. */
(function () {
  'use strict';
  ...
  function renderDrawer(query) {
    ...
    changedBox.appendChild(renderRow(TREE[0].children[0], 'public/index.html'));
  ...
  document.addEventListener('pointerdown', (event) => {
    ...
    if (event.target.closest('#drawerButton')) return;
    closeDrawer();
  });
  ...
  function addToShelf(item) {
    ...
    refreshShelf();
    if (item.external) {
      M.toast('项目外' + (item.kind === 'image' ? '图片' : '文件') + '「' + item.name + '」已复制进任务附件(attachments/<taskId>/)');
    } else {
      M.toast(
  ...
  /* 单一落点区:整个对话列(含输入框)。真实 OS 拖放在浏览器里拿不到绝对路径,
     mock 统一按"外部文件"处理并演示复制语义;产品内 webUtils+relativize 决定内外。 */
  M.bindDropZone(conversation, {
    onItems: (items, event) => {
      const fromTree = !!M.getRefPayload(event.dataTransfer);
      items.forEach((item) => addToShelf(fromTree ? item : { ...item, external: true, meta: (item.meta ? item.meta + ' · ' : '') + '→ 附件' }));
    },
  });

  /* 粘贴截图 → 缩略图 chip(真实剪贴板图片可用) */
