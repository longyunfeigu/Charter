...
- 已知限制（可接受并记录）：cwd 取 launch cwd，用户在 shell 里 `cd` 后不跟踪
  （agent 终端不移动 cwd）；~~跨行折行的长路径 token 不识别~~（am.1 已解除）；
  Windows 的 `file://` pathname 形态（`/C:/…`）留待打包里程碑校准。

## Amendment am.1 — stat 验证路径边界（2026-07-20）

带空格/CJK 的文件名（macOS 截屏 `Screenshot 2026-07-20 at 10.05.32.png`、
`素材/截图 2026.png`）对 FILE_TOKEN 正则不可见，且**没有正则能判定某个空格
是否属于文件名**。借鉴 FanBox 的做法：不猜，问文件系统。

1. **扩展名锚点 + 左扩候选**（`detectWideCandidates`）：以扩展名为锚点向左
   扫描，跨越单空格与 CJK 字符，遇硬边界（引号、管道、glob、中文标点、双
   空格列缝）停止；区间内每个空格/开括号/CJK→ASCII 转换点都是一个候选起点
   （每锚点 ≤6，整行 ≤24，与通道 schema 上限一致）。
2. **新通道 `terminal.statTokens`**（v1，`{ id, tokens[1..24] }` →
   `{ existing: boolean[] }`）：批量只读探测。归一化与封禁复用 openPath 同一
   套（`cwdRelativeToken` + `resolveInsideRoot` + stat isFile），但逐 token
   不抛错——cwd 外/不存在/符号链接逃逸一律 `false`。纯逻辑 `verifyTokens`
   单测覆盖。
3. **最长存在者胜**（`resolveWideMatches` + `mergeFileLinks`）：候选按最长
   优先，第一个真实存在的成为链接，并吞并其覆盖的窄正则命中（`10.05.32.png`
   → 完整截屏名）；无一存在则回退到原正则行为，零回归。
4. **折行拼接**（`readWrappedLine`）：以 `isWrapped` 上下行走拼出逻辑行
   （上限 8 行防病态输出卡 hover），非末行的行尾未写入 padding 格（宽字符
   放不下产生）被剔除，不再向 CJK 路径里注入幻影空格——解除原「跨行折行
   路径不识别」限制，纯 ASCII 长路径折行也顺带修复。
5. **触发与成本**：仅 hover 时运行；每逻辑行一次批量 IPC，结果按行文本
   LRU（64 行/终端）缓存；主进程侧失败不缓存、下次 hover 重试。
