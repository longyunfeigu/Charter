...
| M12-07 | Beta 修复、RC 冻结、Stable 发布 | M12-06 | READY (Beta 4 candidate) / BLOCKED (Stable) | `1.0.0-beta.4` 候选已包含 ADR-0048 更新服务与 updater feed；unsigned 版本自动提醒但仍手动替换。Stable 仍需 Apple/Windows 证书、notarization、真实签名升级资格测试、真实 20-task eval 与 owner sign-off |
...
| 任务 | 交付 | 依赖 | 状态 | 证据 |
| --- | --- | --- | --- | --- |
| M13-01 | 引擎心脏：`terminal-control-service`（六原语——list 含忙闲/read 去 ANSI 滚动缓冲 200KB/send \n→\r+bracketed paste/create 即时返回并在 settle 窗内有序暂存 send/wait 四判定 Agent 回合事件·OSC133 exit 码·静默·正则/kill）+ `tools-terminal.ts` gateway 注册（send 文本过 classifyCommand 动态定级；terminal.* R0-R3 auto-allow 免权限卡，R4 前置拒绝保留；两层封顶；自控拒绝；预算 ≤5 窗 ≤30 send/min）+ ipc-contracts schema | M4,M7 | VERIFIED | ORCH-001/002/006/007/009 VERIFIED；`terminal-control-service.test.ts` + `tools-terminal.test.ts` 覆盖六原语、四种 wait、完成事件竞态、取消清理、非阻塞 create/启动队列、动态风险、免审、R4 拒绝、深度/自控/预算；受管与外部司机 create→send→wait→read 全程零权限卡 |
